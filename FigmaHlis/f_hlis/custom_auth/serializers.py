    ...
    def validate(self, attrs):
        ...
        if 'username' not in validated_data:
            if 'email' not in validated_data and 'phone' not in validated_data:
                raise ValidationError(_('Username, email or phone should have provided.'))
...
class BaseUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id', 'uuid', 'first_name', 'last_name', 'email', 'phone', 'user_type',
                  'password', 'country_code', 'phone_number', 'country',)
